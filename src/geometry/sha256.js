...
export function sha256(input) {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + upper1 + choice + ROUND_CONSTANTS[index] + words[index]) >>> 0;
...
function canonicalize(value) {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new Error('Canonical hashing requires finite numbers.');
  ...
  throw new Error(`Canonical hashing cannot encode ${typeof value}.`);
}
